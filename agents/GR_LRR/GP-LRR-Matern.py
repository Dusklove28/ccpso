...
class EpisodicRewardWrapper(gym.RewardWrapper):
    """Wrapper that delays all rewards until episode end"""
...
class GPRewardModel(nn.Module):
    ...
    def rbf_kernel(self, X1, X2):
        ...
        dist = torch.sqrt(torch.clamp(sq_dist, min=1e-7))

        sqrt_3 = math.sqrt(3)
        scaled_dist = sqrt_3 * dist / length_scale
        K = sigma_f**2 * (1 + scaled_dist) * torch.exp(-scaled_dist)
...
def main():
    class Args:
        # Environment - using simpler environments
        env = "HalfCheetah-v4"
        episodes = 1000
    ...
    # Create evaluation environment (without wrapper)
